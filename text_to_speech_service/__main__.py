...
class MainHandler(RequestHandler):    
    ...
    def post(self):
        ...
        with open('client/source.wav', 'wb') as audio_file:
            if lang1 in voices:
                audio_file.write(
                    text_to_speech.synthesize(', '.join(words), accept='audio/wav',
                                              voice=voices[lang1]))
            else:
                audio_file.write(
                    text_to_speech.synthesize("Sorry, you selected an input language that I don't know yet", accept='audio/wav',
                                              voice=voices['en']))

        with open('client/target.wav', 'wb') as audio_file:
            if lang2 in voices:
                audio_file.write(
                    text_to_speech.synthesize(', '.join(words), accept='audio/wav',
                                              voice=voices[lang2]))
                ...
                audio_file.write(
                    text_to_speech.synthesize("Sorry, you selected an output language that I don't know yet", accept='audio/wav',
                                              voice=voices['en']))
